...
document.addEventListener("DOMContentLoaded", () => {
    ...
    videos.forEach(video => {
        video.setAttribute("poster", "/libre-office/images/video/poster/default.svg")
    });
    ...
    if (articlesContainer) {
        fetch("/libre-office/js/articles.json").then(res => res.json()).then(data => {
    ...
    const remapArticles = (data) => {
        return data.map((item) => {
            return { id: item[0], title: item[1], description: item[2], url: `/libre-office/article/${kebabCase(item[1])}`, open_graph: item[3] }
        })
    }
    ...
    function renderList(items, container, paginatorId, page, isSearch) {
        ...
        pageItems.forEach(item => {
            ...
            if(item.open_graph === true){
                ...
                imageTop.style.backgroundImage = `url(/libre-office/images/open-graph/article/${item.id}.webp)`
                ...
                imageBottom.src = `/libre-office/images/open-graph/article/${item.id}.webp`
